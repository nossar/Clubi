...
| **E-19** ⏳ 🧪 | **O carrossel dos livros do mês na landing** (branch `experimental/landing-carousel`, ainda não mergeada) — a faixa vinho da E-18 passa a mostrar os meses anteriores ao atual, um por vez, num trilho de `scroll-snap` que começa no mês corrente e dá a volta nas duas pontas. **Sem JS é uma pilha**, o mês atual primeiro, com o fio `--clubi-line-invert` entre meses; os controles só aparecem quando o script encontra mais de um mês. Quatro coisas nela não existiam: **o botão contornado sobre vinho** (`.carousel__button` — o espelho do `.button--quiet` da E-18: borda e texto em creme, preenchendo com `--clubi-line-invert` no hover, 6.76 AA), **o olho de mês passado** (`.pick__eyebrow--past`, em `--clubi-ink-muted-on-invert` onde o mês atual segue amarelo — o amarelo é "destaque, novidade" em 6.3, e o passado não é nenhum dos dois), **o contador "4 de 7"** entre os botões (posicional, em `--text-sm` apagado; o mês vai para o leitor de tela numa `span` escondida) e **o sangramento do trilho até a borda do contêiner**, com `gap` igual à goteira mais `--space-4` para o canto da capa inclinada de 1.5° (E-09) não espiar pela margem. Os controles são as palavras "Anterior"/"Próxima" da E-17 com seta tipográfica `aria-hidden`; nada de chevron. Sem autoplay | A alternância e a capa continuam sendo os da E-18; o que a linha acrescenta é ritmo no tempo (3.4 aplicada a meses em vez de a seções) e o par de controles-palavra de 6.3. O trilho é focável e as setas do teclado movem um mês; o anel de foco sobre vinho é amarelo (10.2). `prefers-reduced-motion` troca o deslize por salto e o snap fica, porque é posição e não movimento (8.6). O DOM continua com o mês atual primeiro — `order` negativo inverte só o visual, para crawler e leitor de tela encontrarem o livro do mês antes dos outros | **Experimental e pendente**: a fundadora ainda não viu, e o próprio ADR-18 marca "interatividade além de links" como gatilho de reavaliação — se a branch for mergeada, decidir junto se isso vira ADR. Rever também se um dia houver mais de doze meses a mostrar (`PICK_HISTORY_LIMIT`), porque aí a pergunta é de arquivo, não de carrossel |
