...
| `backend/core/static/brand/elements/` | os 13 SVGs de elemento | `/static/brand/elements/…` **e** empacotados pelo Vite |

Fontes, logotipo e elementos ficam no backend porque os dois lados precisam deles, e em produção é
o Django (com WhiteNoise, que já versiona por hash) que serve tudo.

> **Os elementos moravam em `frontend/src/assets/elements/`** enquanto só a SPA os usava. Mudaram
> de lugar quando a landing (ADR-18) precisou da `estrela-8`: a alternativa era uma segunda cópia,
> que é exatamente o que esta seção existe para impedir. A SPA continua **inlinando** os arquivos
> por `?raw` — `BrandElement` importa de `../../../backend/core/static/brand/elements/`, e o
> inline é o que faz o `currentColor` herdar a cor ao redor (6.1). As páginas renderizadas não têm
> como inlinar um estático, então usam **máscara CSS** (`mask: url(…)` com `background-color`), que
> é a mesma técnica que o logotipo já usava e dá o mesmo resultado: um arquivo, qualquer cor.
...
**Uma exceção, e ela tem nome: o bloco da nota (`.progress__rating`, E-20).** É o único fundo vinho
em nível de bloco do site. Não é zebra de lista — é um bloco só, dentro de uma seção creme, e o que
ele resolve o fio de 1px não resolvia: do progresso para baixo o cartão de leitura são quatro
blocos de texto empilhados sobre o mesmo fundo, e um fio entre dois deles separa sem descansar. O
vinho ali também se paga duas vezes, porque a inversão já é o que deixa a estrela amarela
(`.on-invert .star-rating__fill`) — é o par da própria marca, não uma cor inventada para o painel.
**Se aparecer um segundo bloco pedindo fundo próprio, isto deixa de ser exceção e vira regra nova**,
e a regra a escrever é *quando* um bloco merece fundo — não "todo bloco pode ter um".
...
Os dez elementos também foram extraídos, em `backend/core/static/brand/elements/`, todos com
...
em `backend/core/static/brand/elements/`. Três glifos, não uma dependência.
...
**O quarto lugar chegou — e reescreveu a regra em vez de ser excepcionado (E-17).** A imagem de
uma postagem passou a abrir ampliada, e o painel precisa de um jeito de fechar: é o `×` outra vez.
A E-16 tinha deixado escrito que um quarto caso obrigaria a reescrever a E-07, então aqui está a
redação nova, e ela é mais simples do que a contagem de lugares que vinha sendo feita: **o resíduo
são três glifos com três empregos nomeados — `×` fecha, `seta-cima` sobe, `seta-baixo` desce —, e
usar um deles no emprego que ele já tem não é um caso novo, é a regra funcionando.** Fechar a
estante, fechar o aviso e fechar a imagem são o mesmo verbo; contá-los como três exceções era
contar o lugar em vez do trabalho. O que continua precisando de decisão é um **emprego** novo — um
glifo para "buscar", para "menu", para "compartilhar" —, e aí vale tudo que a E-07 diz: não entra
biblioteca, e o desenho nasce no traço da marca ou não nasce.

A regra 2 não cedeu nada nesse painel: os dois controles de navegação são as palavras "Anterior" e
"Próxima", e o caminho para o arquivo em si é a frase "Ver o arquivo original". O `×` está lá
porque fechar já era emprego dele, não porque um símbolo ficaria mais bonito sobre a foto.
...
| Pontinho entre os dois links do herói (`.pick__more-dot`) | `--space-1` (4px) de diâmetro, `--radius-pill`, `--clubi-highlight` | O menor degrau da grade de 4px da E-10, que é o tamanho em que um ponto separa sem virar elemento. Amarelo porque é a cor que os dois links já têm nesse fundo (3.2, par medido em 6.84) — não entra cor nova para fazer o papel de vírgula. A fila é `align-items: center`, senão um círculo de 4px na linha de base leria como ponto final. **Some abaixo de 479px**, onde os dois links deixam de caber na mesma linha: separador de linha única não tem o que fazer quando a linha virou duas, e lá a própria quebra separa |
| Bloco da nota (`.progress__rating`) | `--space-5` de recheio, `--radius-lg`, fundo `--clubi-bg-invert` | O raio grande é o que 8.2 dá ao que lê como objeto colado na página, e um painel colorido dentro de uma coluna limpa é exatamente isso. A largura não é medida própria: ele herda o `--measure` de `.progress`. O porquê do fundo está em 3.4 e na E-20 |
| Cartão de leitor em "quem já terminou" | `--space-5` de recheio, `--radius-md`, `--clubi-surface` com `--shadow-paper` | A mesma borda de papel do cartão de postagem, e pela mesma razão: numa coluna de textos longos, é a aresta que impede as palavras de uma pessoa de escorrerem para as da seguinte. `--radius-md` e não `--radius-lg` porque é item de lista, não objeto colado |
| Resenha de outro membro | 3 linhas, depois "Ler tudo" | Três linhas é onde uma resenha ainda é uma citação e não um texto: o suficiente para decidir se vale ler, sem que uma pessoa loquaz empurre as outras para fora da tela. O controle só aparece quando o texto de fato excede as três linhas, e isso é **medido no navegador** — contagem de caracteres não sabe onde as palavras quebram |
...
- **Sem ranking, sem ofensiva, sem comparação entre membros.** Não ordene por progresso
  ...
  terminou"** — quem tem `finished_at` e uma nota **ou** uma resenha, com as duas à vista. **Isto é
  a tensão mais próxima do limite desta seção que o site tem**, e foi decidida assim por pedido do
  fundador: uma lista de notas lado a lado *é* comparável. O filtro já exigiu a nota, e isso era
  pior: transformava a avaliação — que esta mesma seção diz ser opcional — em preço de entrada, e
  de quebra apagava da lista quem tinha escrito uma resenha sem dar estrelas. Exigir *alguma* coisa
  é o que resta de preço, e é o que impede a lista de ser uma chamada de presença. O que segura a
  régua, e precisa continuar segurando: a ordem é **alfabética** (nunca por nota, nunca por quem
  terminou primeiro), e **não há média, não há contagem, não há pódio** — nem na lista, nem no link
  que leva até ela.
- **A lista tem tela própria (`/book-of-the-month/readers`, E-20), e isso mudou de lugar mas não de
  princípio.** Esta seção pedia que ela ficasse *dobrada dentro do bloco do livro*, e a razão dada
  continua valendo ao pé da letra: "uma lista de notas que o membro não pediu para ver é outra
  coisa". O que autorizou a mudança foi o **pedido** ter sobrevivido inteiro — a única entrada é um
  link nomeado no herói, nada na Home imprime ou conta a lista, e quem nunca clica nunca vê a nota
  de ninguém. O que se ganhou foi espaço: dentro de um painel de 18rem as resenhas cabiam duas
  pessoas e meia, e a leitura mais longa do clube ficava na mesma tela em que cada membro registra
  as próprias páginas. **O que continua proibido**: um número ao lado do link, um "X pessoas já
  terminaram" em qualquer lugar, ou a lista aparecendo sem alguém ter pedido. Nada no site cobra a
  nota, e apagar as duas (`MonthlyReadingIn.clear_rating` e `review: ""`) tira o membro da lista.
- **Quem não deu nota não ganha estrelas vazias.** Uma fileira de cinco estrelas apagadas ao lado
  de um nome se lê como nota zero, e o backend gastou uma decisão inteira para separar as duas
  coisas (`0` é uma nota; `NULL` é a ausência de uma). No painel, uma linha sem nota imprime só o
  nome. A ausência da fileira *é* o "sem nota".
- **Nota e resenha são opcionais e reversíveis.** Nunca bloqueie um fluxo por falta de avaliação:
  nada no site exige que você dê nota, e nada exige que você escreva.
- **Nota e resenha só aparecem depois de "Terminei este livro" (E-21).** Antes disso o cartão de
  leitura é a barra e o campo de página, e mais nada. **O que faz isso caber nesta seção é o que
  é o portão**: `finished_at`, que é uma *declaração* — o botão está lá em qualquer progresso, e
  ninguém precisa chegar à página N de N para apertá-lo. O que esta seção proíbe é cobrar a
  leitura inteira ("a gente não te cobra ter lido o livro inteiro"), e a contagem de páginas não
  gateia nada. **O custo é real e fica registrado aqui**: quem parou em 60% e quer dizer por quê
  não tem onde, a não ser declarando um fim que não houve. Se isso aparecer como reclamação de
  membro, é esta linha que se reabre, não um ajuste de tela.
- **Esconder não é apagar.** Uma nota e uma resenha escritas antes de "Ainda não terminei"
  continuam na linha e voltam à vista quando a leitura é declarada terminada de novo — que é
  também quando voltam a ser visíveis para o clube, porque `finished_readers` filtra pelo mesmo
  carimbo.
- **Terminar é uma declaração, não só uma inferência**: "Terminei este livro" existe porque
  `Book.pages` é opcional e um livro sem contagem de páginas nunca alcançaria o fim sozinho — e
  porque uma marcação errada precisa ter volta.
...
| **E-07** ✅ | **Decidido (2026-08-27): sem biblioteca de ícones.** Elementos da marca no que é conteúdo, **palavra** no que é controle. Ver 6.3. **Reescrita em 2026-09-02 (E-17):** o resíduo de glifos não se conta por *lugar*, e sim por **emprego** — `×` fecha, `seta-cima` sobe, `seta-baixo` desce. Reusar um deles no emprego que já é dele não é exceção nenhuma | Os dez elementos cobrem bem o significado (nota, anexo, post, livro, vazio) mas não cobrem controle (fechar, menu, seta). Forçar a adaptação seria óbvio; adotar biblioteca traria traço estranho ao lado do desenho à mão. A marca já é verbal — todas as peças resolvem com texto. A contagem por lugar tinha virado uma exceção nova a cada tela (E-15, E-16, E-17) para descrever sempre o mesmo verbo | Rever quando aparecer um **emprego** que os três glifos não cobrem — buscar, menu, compartilhar. Aí volta a pergunta inteira, e a resposta continua não sendo biblioteca: ou nasce no traço da marca, ou é palavra |
...
| **E-15** ⏳ | **Glifo no lugar da palavra num controle repetido** — as setas e o `×` dos quatro lugares da estante, no lugar de "Subir", "Descer" e "Tirar" (ver 6.3) | Pedido do fundador, e é o caso que a E-07 tinha deixado escrito como gatilho: doze rótulos numa grade de quatro colunas de `7rem` é a densidade em que o texto atrapalha. Resolvido do jeito que 6.3 rule 3 manda — desenho no traço da marca, versionado com os demais elementos da marca, nenhuma biblioteca — e a frase que o botão perdeu foi para `aria-label`/`title` nomeando o livro, então 10.5 continua satisfeita | **Pendente de revisão pelo fundador**, junto da E-13 e da E-14. **Gatilho disparado (E-16):** o controle isolado apareceu — o `×` que dispensa o aviso de postagens novas —, e foi resolvido pela regra 3 (`×` é o glifo de *fechar*) em vez de pela densidade. A E-07 continua de pé, mas é o terceiro lugar: rever a redação dela se houver um quarto |
| **E-16** ⏳ | **Glifo num controle isolado** — o `×` que dispensa o aviso de postagens novas (`UnreadNotice`), no lugar do botão "Depois" — e o link "Ver as postagens" saiu da faixa junto com ele (ver 6.3) | Pedido do fundador, e é exatamente o gatilho que a E-15 deixou escrito: um só botão, sem repetição, então a densidade não paga por ele. O que paga é a regra 3, que já reservava o `×` para *fechar* — e fechar um aviso é a ação mais convencionada de uma interface. A frase foi para `aria-label`/`title` ("Dispensar o aviso"), o caminho até as postagens continua sendo palavra uma linha acima (o balão + "Postagens" da E-14, com a mesma contagem), e o arquivo é o `x.svg` que já existia. O alvo de 44px de 10.4 ficou inteiro mas transborda a faixa por margem negativa, e o desenho foi para `0.8rem`: numa faixa de uma linha, o quadrado da estante media mais que o dobro do que ele dispensa | **Pendente de revisão pelo fundador**, junto da E-13, da E-14 e da E-15. **Reescrita feita (E-17)**: o quarto caso apareceu — o `×` que fecha a imagem ampliada — e a E-07 passou a contar empregos em vez de lugares, que é o que esta linha já tinha percebido ao falar em "resíduo com três empregos" |
| **E-17** ⏳ | **Imagem de postagem ampliável, e o `×` que a fecha.** A miniatura da grade vira botão; o clique abre a foto inteira num `<dialog>` modal — papel creme com `--radius-lg` e `--shadow-lifted`, scrim de `rgb(38 22 29 / .88)`, imagem `contain` sem passar dos próprios pixels, "Anterior"/"Próxima" em palavra e "Ver o arquivo original" para o que não couber na tela. Imagem sozinha na postagem deixa de ser cortada: fica na proporção dela até 26rem | Pedido do fundador ("as imagens não são expandíveis, e são cortadas sempre na mesma resolução"). O corte 4/3 continua sendo o que faz quatro fotos lerem como um bloco, mas corte é promessa de que o resto existe em algum lugar. O painel é o "recorte com borda branca" da seção 7, com o raio e a sombra que 8.2 e 8.3 dão a modal; o scrim é o mesmo `ink` quente de que as sombras são feitas, porque cinza neutro sujaria o creme das bordas. `<dialog>` + `showModal()` porque a camada de topo é a única que passa por cima do grão de `body::after` (z-index 100), e porque Escape, armadilha de foco, fundo inerte e a volta do foco para a miniatura vêm de graça e corretos | **Pendente de revisão pelo fundador.** Rever se pedirem zoom/pan dentro do painel, ou se uma postagem passar a aceitar mais de 4 imagens — aí a navegação por palavra vira uma tira de miniaturas |
| **E-20** ⏳ | **O bloco da nota em vinho, e "quem já terminou" como tela.** Duas mudanças de um pedido só: (a) `.progress__rating` ganha fundo `--clubi-bg-invert`, `--radius-lg` e a classe `.on-invert`, virando o único fundo vinho em nível de bloco do site — as estrelas passam a amarelo pela regra que já existia, e junto foram repintados o que estava mixado para creme e não sobrevive ao vinho: a estrela vazia e a legenda para `--clubi-ink-muted-on-invert`, `.button--quiet` ("Remover nota") para amarelo com hover em `--clubi-line-invert`, e `.notice--error`/`--ok` para a tinta creme comum; (b) a lista de quem terminou sai da dobra dentro do cartão de leitura e vira `/book-of-the-month/readers`, com o link "Ver quem já terminou" em amarelo no herói, ao lado de "Ver as escolhas anteriores" | Pedido do fundador, e as duas metades têm a mesma raiz: do progresso para baixo a Home era uma coluna longa de texto sobre um fundo só, separada por fios. O fio separa sem descansar; o bloco de cor descansa. E a lista, que já estava apertada num painel de 18rem desde que passou a trazer resenha, era justamente o que não precisava estar ali — ela é sobre os outros, e o cartão é sobre você. A cor não é nova: a inversão creme ⇄ vinho é o dispositivo de ritmo de 3.4 e o amarelo sobre vinho é o par que 3.2 mede em 6.84, o mesmo do selo e do marca-texto da E-18. Nenhuma trava da seção 9 foi afrouxada no caminho — ordem alfabética, sem média, sem contagem, sem número ao lado do link | **Pendente de revisão pelo fundador.** Dois gatilhos: se um **segundo** bloco pedir fundo próprio, 3.4 precisa ser reescrita como regra ("quando um bloco merece fundo") em vez de ganhar outra exceção; e se alguém pedir uma contagem ao lado do link, ou ordenação por nota na tela nova, aí é a seção 9 inteira que está sendo reaberta — não um ajuste de tela |
| **E-21** ⏳ | **Nota e resenha atrás de "Terminei este livro".** Os dois blocos só são desenhados quando `finished_at` existe; antes disso a Home é barra de progresso, campo de página e o botão de terminar. Junto veio o pontinho amarelo separando os dois links do herói (medida em 8.7) | Pedido do fundador: "antes dele terminar, a página principal é enxuta, apenas com o progresso". **Esta linha contradiz o que a seção 9 dizia** — o documento tinha escrito, com todas as letras, que a resenha não podia ser gateada por terminar o livro, e a nota nunca fora gateada por nada. O que reconcilia as duas coisas é *qual* é o portão: `finished_at` é declaração, não contagem. "Terminei este livro" está disponível em qualquer progresso, então ninguém precisa ler o livro inteiro para ganhar os campos — que é o que a seção 9 e o brandbook proíbem. O que se ganha é a tela enxuta de quem ainda está lendo: a pessoa no meio do livro não é apresentada a dois campos de avaliação que ela ainda não tem o que preencher | **Pendente de revisão pelo fundador**, e com um gatilho mais concreto que o normal: **se aparecer membro querendo registrar o que achou de um livro que largou no meio**, esta é a decisão errada e a saída não é afrouxar o portão — é separar "terminei" de "parei aqui e quero falar". Rever também se o portão algum dia migrar para `pages_read >= total`: aí ele deixa de ser declaração e passa a ser exatamente a cobrança que a seção 9 recusa |
| **E-13** ⏳ | **Dimensões de componente** — largura de logotipo, capa, avatar, trilho de progresso, campo numérico (seção 8.7, Fase 4), mais área de texto, botão de remover imagem e proporção de imagem de post (Fase 5), mais foto de perfil, estrela de nota, slot da estante e capa do histórico (Fase 6), mais campo de busca do cabeçalho e cartão de membro (Fase 7), mais recorte da meia estrela e prévia da nota, mais capa no resultado do catálogo externo (Fase 8), mais painel de quem terminou, balão do cabeçalho e bolinha de não lidas, mais o botão de ordenar/tirar da estante (E-15) e o × que dispensa o aviso (E-16), mais foto da conta e seta do menu de conta no cabeçalho, mais o teto da imagem sozinha no cartão e as medidas do painel da imagem ampliada (E-17) | Nasceu na Fase 4: a seção 8 dava espaçamento, raio, container e breakpoints, mas nenhum tamanho de componente, e a Home precisava deles. Cresceu na Fase 5, na Fase 6, na Fase 7, na nota com meia estrela e na Fase 8 pelo mesmo motivo — formulários de post, depois perfil e estante, depois a busca, depois a metade da estrela, depois a capa vinda da Open Library, precisavam de medidas que a seção 8 não cobria. Cada medida se apoia numa regra que já existia — o alvo de toque de 10.4, a faixa de logotipo de 5.3, o teto de ~240px de 6.2, o `--container-max` | **Pendente de revisão pelo fundador**, junto da E-14. Rever quando um componente novo pedir medida que não caiba nessas |
| **E-18** ⏳ | **A página de apresentação renderizada em `/`** (ADR-18) — a primeira superfície do site que não é nem a SPA nem um formulário de `/accounts/`. Estrutura: **faixa vinho de topo** com o logotipo em creme e a `estrela-8` em amarelo, herói creme cujo `h1` traz **marca-texto amarelo** na segunda metade, faixa vinho com o livro do mês, três passos numerados, convite de fechamento e rodapé vinho — a alternância creme/vinho de 3.4 do começo ao fim. **O topo virou faixa em vez de ganhar um fio de 1px**: o fio resolvia a separação entre logotipo e herói, mas a primeira tela continuava só vinho-sobre-creme, e a faixa resolve as duas coisas de uma vez. **Os dois amarelos são as duas formas que a regra 3 de 3.2 autoriza** — sobre vinho (a estrela, 6.84) e como área grande com texto vinho por cima (o marca-texto, 6.84). Amarelo sobre creme mede 1.38 e está proibido, o que é justamente por que a estrela não pode descer para o herói. O `skip-link` também passou a amarelo com texto vinho, porque ele pousa sobre a faixa vinho e um fundo vinho o tornaria invisível. **A barra de topo não repete os CTAs**: o herói começa logo abaixo dela e o visitante encontrava os mesmos dois botões duas vezes dentro de uma tela. O par reaparece uma vez só, no fim dos três passos, que é onde o leitor já tem a resposta. E a página inteira compartilha uma borda esquerda — logotipo, herói, olho, capa, títulos de seção e rodapé alinham no mesmo eixo; a medida de 68ch limita a *linha* de texto, nunca o bloco (limitá-la no contêiner centralizava o herói e desalinhava tudo). Duas coisas nela são invenção e não reuso: **o botão secundário contornado** (`.button--quiet` — fundo transparente, borda e texto em vinho, preenchendo com `--clubi-tint-wine` no hover) e **o numeral de passo**, que é o `counter()` da própria `<ol>` em Clash laranja, não um selo desenhado | A página não tem respaldo no brandbook porque o brandbook é peça de rede social e não site institucional — mas quase tudo nela é composto de regra que já existia: a alternância creme/vinho é o dispositivo de ritmo de 3.4, a capa é o recorte de borda creme de 7 com as proporções de 8.7, o grão e a rotação de ±1.5° são E-08 e E-09 (a capa é decoração aqui, não controle), o logotipo usa a faixa de 8.7 e os alvos têm os 44px de 10.4. O contorno do botão secundário existe porque a página é divulgada a quem **não** tem conta: dois botões vinho sólidos dariam a "Entrar" e a "Criar conta" o mesmo peso. O numeral sai de 6.3 — onde não há elemento da marca que sirva, a resposta é palavra ou tipo, nunca um desenho novo. **Nenhum elemento gráfico aparece na página**, e não por escolha estética: os dez elementos moram em `frontend/src/assets/`, que o Django não alcança por `{% static %}`, e copiar um para `core/static/brand/` criaria a segunda cópia de um ativo que 2.1 mantém deliberadamente única | **Pendente de revisão pela fundadora**, junto da E-13 — e ela tem uma pendência a mais que as outras: **a copy do herói e dos três passos é placeholder**, marcada como tal no template. Rever a entrada inteira quando esse texto chegar, e de novo se aparecer uma segunda página institucional — aí a barra de topo vira navegação de verdade e deixa de ser dois botões |
| **E-19** ⏳ 🧪 | **O carrossel dos livros do mês na landing** (branch `experimental/landing-carousel`, ainda não mergeada) — a faixa vinho da E-18 passa a mostrar os meses anteriores ao atual, um por vez, num trilho de `scroll-snap` que começa no mês corrente e dá a volta nas duas pontas. **Sem JS é uma pilha**, o mês atual primeiro, com o fio `--clubi-line-invert` entre meses; setas, bolinhas e arrasto só existem quando o script roda. O que nela não existia: **duas setas em glifo** (`‹` `›`) fixas nas bordas do palco, discos de 44px com contorno creme sobre vinho, hover em `--clubi-line-invert` e o clique respondido em amarelo com glifo vinho (a única forma de amarelo que 3.2 permite), **só onde há mouse**: sob `(hover: none) and (pointer: coarse)` elas somem, porque existem para quem não consegue deslizar e no celular cobriam justamente o espiar — **exceção deliberada à regra 2 de 6.3, a pedido da fundadora**: o glifo é `aria-hidden` e o `aria-label` ("Livro anterior"/"Próximo livro") continua sendo a palavra; **as bolinhas** centradas sob o palco, uma por mês, `--space-2` de diâmetro em `--clubi-ink-muted-on-invert` a 50% e a do mês em vista em creme cheio — cada uma é um botão de 44px de altura por **24px de largura**, abaixo dos 44px de 10.4 porque treze meses têm de caber numa linha de 390px, e aceitável porque é o terceiro caminho para o mesmo mês (setas, deslize); **o espiar dos vizinhos**: o trilho ganha `padding` de goteira + `--space-6` (`--space-8` a partir de 1024px) em cada lado, de modo que uma fatia do mês anterior e do seguinte fica à vista, **ofuscada e não cortada** — gradiente do próprio vinho sobre a zona da borda e `opacity: .55` em todo mês que não é o em vista; e **o arrasto com o mouse** (`cursor: grab`), que desliga o snap enquanto a mão segura e solta pelo mesmo `go()` das setas. **O olho de mês passado** (`.pick__eyebrow--past`) continua em `--clubi-ink-muted-on-invert`. Sem autoplay | A alternância e a capa continuam sendo os da E-18; o que a linha acrescenta é ritmo no tempo (3.4 aplicada a meses em vez de a seções). O custo registrado: o mês em vista fica recuado da borda esquerda da página pelo tamanho do espiar — a página deixa de ter um eixo único nessa faixa, que passa a ler como um palco centrado. O trilho é focável e as setas do teclado movem um mês; Enter/Espaço ativam setas e bolinhas; todo anel de foco sobre vinho é amarelo (10.2). `prefers-reduced-motion` troca o deslize por salto — inclusive ao soltar um arrasto, ao clicar numa bolinha e no fade do mês — e o snap fica, porque é posição e não movimento (8.6). O DOM continua com o mês atual primeiro — `order` negativo inverte só o visual, para crawler e leitor de tela encontrarem o livro do mês antes dos outros; um arrasto não dá a volta (o deslize de toque também não), só as setas | **Experimental e pendente**: a fundadora ainda não viu, e o próprio ADR-18 marca "interatividade além de links" como gatilho de reavaliação — se a branch for mergeada, decidir junto se isso vira ADR, e se a exceção dos glifos fica só aqui ou reescreve a regra 2 de 6.3. Rever também se um dia houver mais de doze meses a mostrar (`PICK_HISTORY_LIMIT`): as bolinhas dobram de linha em vez de estourar, mas aí a pergunta é de arquivo, não de carrossel |
...
3. ~~Exportar SVG dos 10 elementos~~ — **feito**, `backend/core/static/brand/elements/`, 97 KB (5.6).
