...
**Current state: the whole backend is done — models, admin, auth and the API. The frontend has Fases 4 to 8 (Home, posts, profiles, busca, catálogo externo), so every screen the guide's route table names is mounted and every phase of frontend work is closed. What is left is Fase 9, the deploy.**
