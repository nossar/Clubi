...
**Current state: the whole backend is done — models, admin, auth and the API. The frontend has Fases 4 to 7 (Home, posts, profiles, busca), so every screen the guide's route table names is mounted. What is left is Fase 8 (deploy) and Fase 9 (the external-book autocomplete).**
