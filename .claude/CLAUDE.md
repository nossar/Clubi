...
Ruff: line-length 100, migrations excluded. `select` is unset, but do **not** read that as the old `E4`/`E7`/`E9`/`F` default — on the pinned ruff (0.16.2) the default set is much wider, and `B`, `S`, `BLE`, `EXE` and `RUF` all fire. That is why `per-file-ignores` has to silence `RUF012` on `models.py`, `admin.py` and `schemas.py`. Trailing-newline enforcement comes from `ruff format`, not from the linter — `ruff check` alone will not catch it, so run both. And note the `&&` in the `lint` target: while `ruff check` fails, `ruff format` never runs at all.
